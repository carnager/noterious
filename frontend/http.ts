...
function mergeClientHeaders(headers?: HeadersInit): Headers {
  const merged = new Headers(headers);
  const clientID = currentClientInstanceId();
  if (clientID) {
    merged.set(clientIDHeaderName, clientID);
...
export function unscopedRequestInit(options?: RequestInit): RequestInit {
  return {
    ...options,
    headers: mergeClientHeaders(options && options.headers ? options.headers : undefined),
  };
}
...
export async function fetchJSONUnscoped<T>(url: string, options?: RequestInit, suppressAuthEvent = false): Promise<T> {
