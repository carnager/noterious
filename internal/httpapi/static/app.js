...
(() => {
  ...
  function renderSettingsForm(state, els) {
    ...
    els.settingsRuntimeListenAddr.value = state.serverMeta ? String(state.serverMeta.listenAddr || "").trim() || "(unknown)" : "(unknown)";
    ...
    els.settingsRuntimeHealth.value = !state.serverMeta ? "(unknown)" : vaultHealth && vaultHealth.healthy ? "Healthy" : String(vaultHealth && vaultHealth.message ? vaultHealth.message : "Unavailable");
